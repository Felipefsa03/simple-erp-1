...
export function Configuracoes({ onNavigate }: ConfiguracoesProps) {
  ...
  useEffect(() => {
    ...
    const loadGlobalConfig = async () => {
      try {
        const { supabase, isSupabaseConfigured, getSupabaseSession } = await import('@/lib/supabase');
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-3 py-1 text-xs font-bold rounded-full ${twoFactorEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
            ...
            {/* 2FA Setup Modal */}
            <Modal isOpen={showTwoFAModal} onClose={() => { setShowTwoFAModal(false); setTwoFAQrCode(''); setTwoFASecret(''); }} title="Configurar Autenticação em Dois Fatores" maxWidth="max-w-md">
              <div className="space-y-4">
                <div className="text-center">
                  <p className="text-sm text-slate-600 mb-4">
                    Escaneie o QR Code abaixo com seu aplicativo autenticador (Google Authenticator, Authy, Microsoft Authenticator, etc.)
                  </p>
                  ...
                  {twoFASecret && (
                    <div className="mt-4 p-3 bg-slate-100 rounded-lg">
                      <p className="text-xs text-slate-500 mb-1">Ou digite esta chave manualmente:</p>
                      <p className="font-mono text-sm font-bold text-slate-700">{twoFASecret}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3">
                  <button 
                    onClick={() => { setShowTwoFAModal(false); setTwoFAQrCode(''); setTwoFASecret(''); }}
                    className="flex-1 px-4 py-2 bg-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-300 transition-colors"
                  >
                    Cancelar
                  </button>
                  <button 
                    ...
                    className="flex-1 px-4 py-2 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {twoFactorLoading && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                    Verificar e Ativar
                  </button>
                </div>
              </div>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (existing) {
                        ...
                        ({ error } = await supabase
                          .from('integration_config')
                          .insert({ clinic_id: SYSTEM_GLOBAL_CLINIC_ID, mp_access_token: integrationForm.mp_access_token, mp_public_key: integrationForm.mp_public_key }));
                      }
