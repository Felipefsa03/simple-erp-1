...
import { LogIn, Sparkles, Menu, AlertCircle, Eye, EyeOff, Download } from 'lucide-react';
...
export default function App() {
  ...
  const [publicRoute, setPublicRoute] = useState<'landing' | 'book-online' | 'anamnese-form'>('landing');
  const [publicClinicId, setPublicClinicId] = useState<string>('clinic-1');
  const [publicAnamneseToken, setPublicAnamneseToken] = useState<string>('');
  // ---- Ultra-Stable Atomic Selectors ----
  const navAppointmentId = useClinicStore(s => s.navigationContext?.appointmentId || null);
  const navFromModule = useClinicStore(s => s.navigationContext?.fromModule || null);
  ...
  const handleInstallPwa = async () => {
    ...
    if (isIos && !isInstalled) {
      toast('No iPhone, toque em Compartilhar e depois "Adicionar a Tela de InÃ­cio".', 'info');
    }
  };
  ...
  useEffect(() => {
    let cancelled = false;
    // Buscamos os dados reais via getState para evitar re-renderizações por referência de array
    ...
    const processQueue = async () => {
      await new Promise(r => setTimeout(r, 200)); // Delay de proteção
      if (cancelled) return;
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 text-center space-y-4">
                ...
                {!isInstalled && (canInstall || isIos) && (
                  <button
                    ...
                    className="w-full text-xs font-bold text-cyan-700 bg-cyan-50 border border-cyan-100 rounded-xl py-2 hover:bg-cyan-100 transition-colors flex items-center justify-center gap-1"
                  >
            ...
            {!isInstalled && (canInstall || isIos) && (
              <div className="bg-cyan-50 border-b border-cyan-100 px-4 py-2 flex items-center justify-between gap-3">
                <p className="text-xs text-cyan-800 font-medium">
                  Instale o LuminaFlow no celular para acesso rÃ¡pido e experiÃªncia de app.
                </p>
                <button
                  onClick={handleInstallPwa}
                  className="px-3 py-1.5 text-xs font-bold rounded-lg bg-cyan-600 text-white hover:bg-cyan-700 whitespace-nowrap"
                >
                  Instalar
                </button>
              </div>
