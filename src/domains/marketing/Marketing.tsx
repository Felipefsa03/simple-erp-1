...
export function Marketing() {
  ...
  const updateWhatsAppStatus = (index: number, status: 'pending' | 'opened' | 'sent' | 'skipped') => {
    setWhatsAppStatuses(prev => {
      const base = prev.length ? [...prev] : Array.from({ length: whatsAppLinks.length }, () => 'pending');
      base[index] = status;
      return base;
    });
  };
  ...
  useEffect(() => {
    if (whatsAppSessionActive && whatsAppLinks.length > 0 && whatsAppQueueIndex >= whatsAppLinks.length) {
      setWhatsAppSessionActive(false);
      appendWhatsAppLog('Fila concluida.');
    }
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {activeTab === 'campaigns' && (
          <button
            ...
            className="bg-cyan-600 text-white px-4 py-2 rounded-xl font-bold text-sm hover:bg-cyan-700 shadow-lg shadow-cyan-200"
          >
            + Criar Campanha
          </button>
        )}
      ...
      {activeTab === 'campaigns' && (
        <div className="space-y-4">
          {campaigns.map(c => (
            <div key={c.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col md:flex-row md:items-center gap-4">
              <div className="flex items-center gap-4 flex-1">
                <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center",
                  c.type === 'facebook' ? 'bg-blue-50 text-blue-600' :
                    c.type === 'email' ? 'bg-purple-50 text-purple-600' : 'bg-green-50 text-green-600'
                )}>
                  {c.type === 'facebook' ? <Facebook className="w-6 h-6" /> : c.type === 'email' ? <Mail className="w-6 h-6" /> : <MessageSquare className="w-6 h-6" />}
                </div>
                <div>
                  <p className="font-bold text-slate-900 flex items-center gap-2">
                    {c.name}
                    <span className={cn("text-[10px] uppercase px-2 py-0.5 rounded-full font-bold", c.status === 'active' ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600")}>
                      {c.status}
                    </span>
                  </p>
                  <p className="text-xs text-slate-500 capitalize">Canal: {c.type}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-6 text-center bg-slate-50 p-3 rounded-xl">
                <div><p className="text-lg font-bold text-slate-900">{c.sent}</p><p className="text-[10px] text-slate-400 uppercase font-bold">Alcance</p></div>
                <div><p className="text-lg font-bold text-cyan-600">{c.opened}</p><p className="text-[10px] text-slate-400 uppercase font-bold">Engajamento</p></div>
                <div><p className="text-lg font-bold text-emerald-600">{c.converted}</p><p className="text-[10px] text-slate-400 uppercase font-bold">Conversões</p></div>
              </div>
            </div>
          ))}
          {campaigns.length === 0 && (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 border-dashed">
              ...
              <p className="text-slate-500 font-medium">Você ainda não tem campanhas rodando.</p>
            </div>
          )}
        </div>
      )}
